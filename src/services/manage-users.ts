...
async function callUserManagementApi<T>(body: Record<string, unknown>, options: { idToken?: string } = {}): Promise<T> {
  const token = options.idToken || await getIdToken();
  ...
  try {
    ...
    if (!response.ok) {
      const fallback = text || `User management request failed (${response.status})`;
      const reason = typeof parsed === 'object' && parsed !== null && 'error' in parsed
        ? String((parsed as { error?: unknown }).error || fallback)
        : fallback;
      throw new Error(reason);
...
export async function addManageUser(input: AddManageUserInput): Promise<AddManageUserResponse> {
  return callUserManagementApi<AddManageUserResponse>({
    action: 'addUser',
    email: input.email,
    role: input.role,
...
export async function recordManagedUserLogin(input: {
  email: string;
  role: ManageUserRole;
  ...
  if (input.classCode != null) {
    payload.classCode = input.classCode;
  }

  return callUserManagementApi<UserManagementResponse>(payload, { idToken: input.idToken });
}
...
export async function updateManagedUser(input: {
  ...
  return callUserManagementApi<UserManagementResponse>({
    action: 'updateUser',
    email: input.email,
    name: input.name,
    role: input.role,
...
export async function disableManagedUser(email: string): Promise<UserManagementResponse> {
  return callUserManagementApi<UserManagementResponse>({
    action: 'disableUser',
    email,
  });
...
export async function moveManagedStudent(input: {
  email: string;
  ...
  return callUserManagementApi<UserManagementResponse>({
    ...
    targetClassCode: input.targetClassCode ?? null,
  });
}

export async function removeManagedStudentFromClass(email: string): Promise<UserManagementResponse> {
  return callUserManagementApi<UserManagementResponse>({
  ...
  const promise = (async () => {
    ...
    const users: ManageUserRecord[] = usersSnapshot.docs.flatMap((doc) => {
      ...
      return [{
        ...
        lastLogin: data.lastLogin ?? data.lastLoginAt ?? data.lastSignInTime ?? data.lastSeenAt,
