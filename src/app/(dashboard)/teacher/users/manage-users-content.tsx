...
export function ManageUsersContent() {
  ...
  const handleAddUserSubmit = async () => {
    ...
    try {
      ...
      setAddUserEmail('');
      setAddUserClassCode('');
      setAddUserFeedback({ type: 'success', message: `${email} was added successfully. Reloading user list...` });

      // Ensure stats/cards reflect newest counts even if backend propagation is slightly delayed.
      window.setTimeout(() => {
        window.location.reload();
      }, 500);
    } catch (err) {
  ...
  const handleEditUser = async (item: ManageUserRecord) => {
    ...
    const targetEmail = String(item.email || item.id || '').trim();
    ...
    try {
      await updateManagedUser({
        ...
        role: nextRole as ManageUserRole,
  ...
  const handleDeleteUser = async (item: ManageUserRecord) => {
    ...
    const targetUserEmail = String(item.email || item.id || '').trim();
    ...
    const confirmed = window.confirm(`Delete user "${safeName}"?\n\nThis will disable the account and remove it from the active user list.`);
    if (!confirmed) return;

    try {
      await disableManagedUser(targetUserEmail);
      await loadData({ showLoading: true, forceFresh: true });
    } catch (err) {
  ...
  const handleConfirmMoveStudent = async () => {
    ...
    const targetEmail = String(movingStudent.email || movingStudent.id || '').trim();
    if (!targetEmail) {
      setError('Missing student email.');
      return;
    }
    ...
    try {
      ...
      await moveManagedStudent({
        email: targetEmail,
        targetClassId: targetClass.id,
        targetClassCode: targetClass.code || targetClass.id,
      });
  ...
  const handleRemoveStudentFromClass = async (student: ManageUserRecord, currentClass: ManageClassRecord) => {
    ...
    const targetEmail = String(student.email || student.id || '').trim();
    if (!targetEmail) {
      setError('Missing student email.');
      return;
    }
    ...
    try {
      ...
      await removeManagedStudentFromClass(targetEmail);
      await loadData({ showLoading: true, forceFresh: true });
    } catch (err) {
